<?php
/**
 * Plugin Name: MultiParcels/Noparcels Shipping For WooCommerce
...
class MultiParcels
{
    ...
    /** @var string */
    public $version = '1.30.30';
    /** @var MP_Options */
    public $options;
    /** @var MP_Api_Client */
    public $api_client;
	/** @var MP_Real_Time_Prices_Api */
	public $real_time_prices_api;
    /** @var MP_Locations */
    public $locations;
    /** @var MP_Carriers */
    public $carriers;
    ...
    function __construct()
    {
        ...
        add_action('rest_api_init', function () {
            register_rest_route('custom/v1', '/shipment-status', [
                    'methods'  => 'POST',
                    'callback' => 'handle_shipment_status_webhook',
                    'permission_callback' => '__return_true', // make public (or secure later)
            ]);
        });
        function handle_shipment_status_webhook($request) {
            //Get JSON body
            $data = $request->get_json_params();
            $new_status = $data['data']['new_status'];
//            $order = wc_get_order( $data['data']['shipment_order_reference'] );
            $order = wc_get_order( $order_id );
            if (class_exists( \Automattic\WooCommerce\Utilities\OrderUtil::class )) {
                if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
                    $existing = $order->get_meta('multiparcels_carrier_shipment_status');
                    ...
                    $order->update_meta_data('multiparcels_carrier_shipment_status', $existing);
                    $order->save();
//                    $order->update_meta_data('multiparcels_carrier_shipment_status', $data['data']['new_status']);
                } else {
                    //OLD
                    //update_post_meta($order_id, 'multiparcels_carrier_shipment_status', $data['data']['new_status']);
                    $existing = get_post_meta($order_id, 'multiparcels_carrier_shipment_status', true);
                    if (!is_array($existing)) {
                        $existing = $existing ? [$existing] : [];
                    }
                    $existing[] = [
                    ...
                    update_post_meta($order_id, 'multiparcels_carrier_shipment_status', $existing);
//                    $existing[] = $new_status;
                }
            }else {
                //update_post_meta($order_id, 'multiparcels_carrier_shipment_status', $data['data']['new_status']);
...
//            error_log($data['data']);
        ...
        add_action('admin_post_nopriv_serve_label', 'mp_serve_label');
        ...
        function backup_shipments_labels($true, $hook_extra) {
            if (isset($hook_extra['plugin']) && strpos($hook_extra['plugin'], 'multiparcels-shipping-for-woocommerce') !== false) {
                $plugin_dir = WP_PLUGIN_DIR . '/multiparcels-shipping-for-woocommerce/shipments_labels';
...
//            2
//        );
        add_action('woocommerce_thankyou', 'clear_pickup_localstorage');
        function clear_pickup_localstorage() {
            ?>
            <script>
                localStorage.removeItem('mp_selected_pickup_point_block');
                localStorage.removeItem('mp_selected_pickup_point');
                localStorage.removeItem('mp_selected_pickup_point_text_block');
                localStorage.removeItem('mp_selected_pickup_point_text');
            </script>
            <?php
        }
//        add_action( 'woocommerce_new_order', 'my_custom_order_created_action' );
        add_action( 'woocommerce_store_api_checkout_order_processed', 'my_custom_order_created_action' );
        function my_custom_order_created_action($order_id){
            wc_clear_notices();
            if(WC()->session) {
                $order = wc_get_order( $order_id );
                $chosen_shipping_methods = WC()->session->get('chosen_shipping_methods');
                if (!is_array($chosen_shipping_methods) || empty($chosen_shipping_methods[0])) {
                    return $order;
                }
                $chosen_shipping_method = (string) $chosen_shipping_methods[0];
                $shipping_method = $chosen_shipping_method;
                $shipping_method = $chosen_shipping_method;
                $check = 'multiparcels_';
                $check1 = false;
                ...
                $word_siuntos_autobusais_courier = 'siuntos_autobusais_courier';
                $multiparcels_pickup_location_value = WC()->session->get( 'multiparcels_pickup_location_value');
                $storage_location_identifier = multiparcels_get_storage_shipping_method_location_identifier($chosen_shipping_method);
                if ($storage_location_identifier) {
                    $multiparcels_pickup_location_value = $storage_location_identifier;
                }
                if (strpos($chosen_shipping_method, $word_siuntos_autobusais_courier) !== false) {
                ...
                if (strpos($chosen_shipping_method, $word_pickup_point) !== false || strpos($chosen_shipping_method, $word_terminal) !== false) {
                    if (!$multiparcels_pickup_location_value) {
                        if(!MultiParcels()->options->get( 'show_pickup_points_notice') || MultiParcels()->options->get( 'show_pickup_points_notice') == 'yes') {
                            wc_add_notice( __("Please select the pickup location", 'multiparcels-shipping-for-woocommerce'), 'error' );
                        }
...
//            echo json_encode(['content' => $additionalContent]);
        ...
        function multiparcels_is_storage_shipping_method_enabled($shipping_method)
        {
            if (!$shipping_method) {
                return false;
            }
            $parts = explode(':', $shipping_method);
            $method_id   = isset($parts[0]) ? sanitize_text_field($parts[0]) : '';
            ...
            $services = isset($settings['default_services']) ? $settings['default_services'] : [];
        ...
        function multiparcels_get_storage_shipping_method_location_identifier($shipping_method)
        {
            if (!$shipping_method) {
                return '';
            }
            ...
            return sanitize_text_field($settings['storage_location_identifier']);
        }
        function multiparcels_ajax_is_storage_shipping_method()
        {
            if (!wp_verify_nonce($_POST['nonce'], 'wc_store_api')) {
                echo 0;
                wp_die();
            ...
            echo multiparcels_is_storage_shipping_method_enabled($selected_value) ? 1 : 0;
            wp_die();
        }
        ...
        function multiparcels_save_pickup_location() {
            ...
            $value = isset($_REQUEST['multiparcels_pickup_location_value'])
                    ? sanitize_text_field(wp_unslash($_REQUEST['multiparcels_pickup_location_value']))
                    : '';
            if ($value) {
        ...
        add_action('wp_ajax_multiparcels_is_storage_shipping_method', 'multiparcels_ajax_is_storage_shipping_method');
        add_action('wp_ajax_nopriv_multiparcels_is_storage_shipping_method', 'multiparcels_ajax_is_storage_shipping_method');
...
//        });
    ...
    {
//        $lang_dir = WP_LANG_DIR . '/plugins/multiparcels-shipping-for-woocommerce';
//
//        if ( is_dir( $lang_dir ) ) {
...
//        add_filter( 'auto_update_plugin', '__return_true' );
    ...
    public function enqueue_scripts()
    {
        ...
        if (function_exists('is_checkout') && is_checkout()) {
			...
			$pickup_points_js_file = 'public/js/pickup-points.js';
			$pickup_points_js_path = plugin_dir_path(__FILE__) . $pickup_points_js_file;
			$pickup_points_js_url  = plugins_url('/' . $pickup_points_js_file, __FILE__);

			$pickup_points_js_version = file_exists($pickup_points_js_path)
					? filemtime($pickup_points_js_path)
					: MultiParcels()->version;

			$pickup_points_js_dependencies = ['jquery'];

			if (function_exists('has_block') && has_block('woocommerce/checkout')) {
				$pickup_points_js_dependencies = [
						'jquery',
						'wp-element',
						'wp-data',
						'wc-blocks-checkout',
			...
			wp_enqueue_script(
					'wp-mp-shipping-js',
					$pickup_points_js_url,
					$pickup_points_js_dependencies,
					$pickup_points_js_version,
					true
			);
...
//                    'nonce'    => wp_create_nonce('wp_rest'),
                    ...
                    'text'                           => [
                            ...
                            'loading_more'              => __('Loading more results...', 'multiparcels-shipping-for-woocommerce'),
    ...
    private function includes()
    {
		...
		$this->real_time_prices_api = include_once 'includes/class-mp-real-time-prices-api.php';
		include_once 'includes/class-mp-webhooks.php';
		...
		include_once 'includes/abstracts/abstract-wc-mp-real-time-shipping-method.php';
        // Dpd
		...
		include_once 'includes/shipping/class-wc-mp-lp-express-real-time-courier-shipping.php';
		...
		include_once 'includes/shipping/class-wc-mp-post-lt-real-time-courier-shipping.php';
        // Unisend
        ...
        include_once 'includes/shipping/class-wc-mp-fedex-real-time-courier-shipping.php';
        // GLS
...
//        include_once 'includes/shipping/class-wc-mp-siuntos-autobusais-pickup-point-shipping.php';
    ...
    public function load_plugin_textdomain()
    {
        load_textdomain('multiparcels-shipping-for-woocommerce',
                plugin_dir_path( __FILE__ ) . '/languages/multiparcels-shipping-for-woocommerce-' . get_locale() . '.mo');
